...
Json::Json(const std::string &s) {
    ...
    } else throw WrongJson("Illegal string!");
...
bool Json::is_empty() const {
    return object.empty() && array.empty();
}

size_t Json::getArrSize() const {
    if (is_array()) {
        return array.size();
    } else throw WrongJson("It is not an array!");;
}

std::pair<std::experimental::any, unsigned int> getValueAndLen(const std::string &s, unsigned int start) {
...
std::vector<std::experimental::any> Json::getArray(const std::string &s, unsigned int start) const {
...
std::map<std::string, std::experimental::any> Json::getObject(const std::string &s, unsigned int start) const {
